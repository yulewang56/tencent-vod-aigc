...
function openEditor(node) {
  ...
  state.refreshTimeline();
  runtime.fitView();
  loadBackgroundAsset(String(assetWidget?.value || ""), runtime, viewport);
...
function createThreeRuntime(mainHost, monitorHost, state, onManipulated) {
  ...
  const monitorRenderer = new THREE.WebGLRenderer({ antialias: true, alpha: false });
  ...
  const ambient = new THREE.HemisphereLight(color("--cp-surface"), color("--cp-border-strong"), 2.1);
  const key = new THREE.DirectionalLight(color("--cp-surface"), 2.5);
  key.position.set(5, 9, 4);
  key.castShadow = true;
  mainScene.add(ambient, key);
  ...
  function updateAnimatedObjects(time) {
    for (const item of state.scene.objects) {
      ...
      if (!root || (transform.dragging && transformBinding?.object === root)) continue;
      root.position.fromArray(evaluateTrack(item.motion_track, time));
      root.rotation.set(...vec3(item.rotation));
      root.scale.set(...vec3(item.scale, [1, 1, 1]));
      if (item.type === "actor") {
        const tangent = trackTangent(item.motion_track, time);
        if (Math.hypot(tangent[0], tangent[2]) > 0.01) root.rotation.y = Math.atan2(tangent[0], tangent[2]);
        animateActor(root, item.motion === "walk" ? time * state.duration : 0, item.motion === "walk");
      }
    }
  }
  ...
  function render(time) {
    ...
    const observation = state.followCut
      ...
      : (() => {
          const camera = state.cameraRig.cameras.find((item) => item.id === state.observationCameraId)
...
function loadBackgroundAsset(path, runtime, viewport) {
  if (!path) return;
  const cleanPath = path.split("?")[0].split("#")[0];
  const extension = cleanPath.includes(".") ? cleanPath.split(".").pop().toLowerCase() : "";
  if (extension === "spz") {
    const notice = element("div", "vod-previs__hint vod-previs__asset-notice");
    const detail = textElement(
      "div",
      "",
      "请使用 ComfyUI 原生 Preview Splat 节点查看该资产；轨迹编辑仍可正常进行。",
    );
    notice.append(
      textElement("div", "", "SPZ 背景不会在此 WebGL 编辑器中伪加载。"),
      detail,
      button("Preview Splat 说明", () => {
        detail.textContent = "关闭编辑器后，将同一 SPZ 路径连接到 ComfyUI 的 Preview Splat 节点进行原生预览。";
      }),
    );
    viewport.appendChild(notice);
    return;
  }
  if (!["glb", "gltf", "obj", "ply"].includes(extension)) return;
  ...
  if (extension === "glb" || extension === "gltf") {
    ...
    new OBJLoader().load(assetUrl, (object) => {
      notice.remove();
      runtime.addAsset(object);
    }, undefined, onError);
  } else {
    new PLYLoader().load(assetUrl, (geometry) => {
