...
| 模型 | 腾讯接入层引用方式 | 本节点语法 |
|---|---|---|
| H3 (Hailuo) | 「图1/视频1/音频1」顺序引用（`FileInfos.Text` 仅 PixVerse 生效） | `@N` → 图N |
| VS (SeeDance) | 腾讯层无 `Text` 绑定；顺序引用 | `@N` → 图N |
| PixVerse | `FileInfos.Text` + `@名称` 绑定 | （无节点） |
| Kling / Vidu / MiniMax 官方直连 | `<<<image_1>>>` / `@主体Id` / `content[]`+role | （无节点） |

`@N` 是本节点/腾讯接入层的易用语法，不是模型官方原生协议——模型原生引用语法
（如火山 Seedance 的 `@Image1`）未经腾讯接入层文档化，勿混用。
