...
### Added
- **参考机位重投影校准**：使用家具落地点的 2D-3D 对应联合优化机位位置、目标点和 FOV，
  自动比较原方向、镜像方向与约定方向；Manifest 和报告输出观测数、重投影误差与可靠性
- **预演还原质量门槛**：新增 `usable / limited` 等级，综合参考机位、家具穿插、教室门窗/板面
  覆盖和图像顺序冲突率给出限制原因，区分“任务成功生成”与“结果足以用于预演”
- **教室建筑覆盖指标**：逐项统计窗、门、黑板/白板和有图像证据的建筑锚点；同墙、同类、
  几何重合且 bbox 重叠或相接的窗格分段会保守合并

### Changed
- 家具行列排布保留图像中的非均匀左右/前后间距，仅做满足净距所需的最小位移；无图像证据的
  推断家具若与有证据同类实例近乎重合，会优先保留有证据实例
- 门窗、板面和桌椅允许视觉模型先返回零厚度，再由类别物理先验修正，避免在先验生效前被丢弃
- 图像顺序质量仅比较观测与投影都相差至少 4% 的对象，并在有效冲突率超过 10% 时降级，
  避免将近似并列对象的浮点符号变化误报为布局反转
- 课堂参考图离线复算达到 5 组桌椅、3 个有效窗组、2 个板面、0 非配对穿插、0 越界；
  参考机位误差 7.86%，有效图像顺序冲突率 4.4%，结果等级为 `usable`

## [v1.27.0] - 2026-08-21

### Added
- **桌椅全局布局求解器**：按主图观测对桌椅做保守实例去重、双向 ID 优先的一对一配对、
  图像行列聚类和组合单元排布；配对桌椅作为整体约束到房间边界，不再依赖逐椅子最近邻推开
- **布局质量指标**：Manifest 和重建报告新增输入/输出家具数、去重数、桌椅配对数、深度行数、
  求解前后碰撞数，以及桌子和椅子的最小净距；残余碰撞会明确写入警告
- **导演台模型选择**：图片重建面板可选择混元 T1 Vision 或 Vision 1.5

### Changed
- 图片结构提示要求先逐一盘点可见桌面和椅面，禁止用一个大框概括整排家具，并输出稳定
  `instance_id / paired_instance_id`
- 导演台和独立重建节点默认使用 `hunyuan-t1-vision-20250916`；在课堂参考图实测中识别到
  7 张桌子和 7 把椅子，完成 7 组一对一配对，非配对碰撞从 22 组降为 0
- 家具去重仅合并相同实例 ID 或图像观测近乎完全重合的对象，避免把透视遮挡造成的 bbox
  重叠误判成重复实例

## [v1.26.0] - 2026-08-21

### Added
- **导演台内图片重建**：场景来源新增 `Image Reconstruction`，可在导演台内上传 1-3 张图片、
  设置空间类型/已知宽度/对象上限/补充约束并确认费用；后台异步调用混元视觉，完成后直接载入
  原生可编辑白模，无需先运行独立节点再手工传递 JSON
- **安全替换与历史**：图片重建可只替换环境并保留现有人物、多机位和 CUT；空场景自动采用参考图
  机位，整次导入纳入撤销/重做，失败时保持原场景
- **图像空间布局约束**：混元视觉输出每个实例的 `image_bbox / floor_contact`，本地用相机射线与
  有界图像布局混合校准桌椅间距和深度行列，并对重复观测、重叠桌椅执行去重和物理净距分离
- **窗台高度校准**：门窗/板面结合墙面、图像下沿和物理高度先验定位；不可信参考相机会自动回中，
  防止所有对象被投影压缩到房间边界

### Changed
- 重建报告显示使用主图约束的对象数量，并将过多修正信息折叠为摘要
- `3D 白模预演台` 的 `scene_source` 新增持久化的 `Image Reconstruction` 选项；原独立
  `图片转可编辑 3D 白模` 节点继续保留，适合批量或可组合工作流

## [v1.25.0] - 2026-08-21

### Added
- **类别物理先验**：自动识别并修正视觉模型把桌面/座面厚度误当成整体高度的问题；课桌、椅子、
  门、窗和板面使用各自的合理尺寸范围，并把落地家具统一吸附到地面
- **墙面归属约束**：混元视觉为门、窗、黑板和白板输出 `wall`，本地几何层将同墙对象吸附到
  一致平面，统一墙面朝向、厚度和沿墙边界，避免每扇窗独立选择最近墙面
- **参数化导演代理**：课桌显示为桌面+四腿，椅子显示为座面+椅背+四腿，窗户显示为窗框+窗面；
  每件家具仍保持单一层级对象，可整体选择、变换、配色、参与轨迹和撤销

### Changed
- 重建提示不再包含固定房间尺寸和参考机位数值，避免视觉模型照抄 `8×10×3m` 等示例；
  增加全包围盒高度、墙面一致性和输出前物理自检要求
- 预演场景解析保留 `semantic` 元数据，使参数化类别造型在保存、执行和重新打开后保持一致
